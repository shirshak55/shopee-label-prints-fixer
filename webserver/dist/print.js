...
export var Vendor;
(function (Vendor) {
    Vendor["Sevenoneone"] = "Sevenoneone";
    Vendor["Family"] = "Family";
    Vendor["Okmart"] = "Okmart";
    Vendor["Lai"] = "Lai";
})(Vendor || (Vendor = {}));
export async function order_details(indexPath) {
...
export async function makePdf(vendor, inputLabelDirectory, outputDir, indexPath, openOnceDone) {
    ...
    let output = {
        stats: 0,
        count: idata.filter((v) => `${v.type}`.toLowerCase() === `${vendor}`.toLowerCase() ||
            (vendorMap[`${vendor}`.toLowerCase()] || "").toLowerCase() === `${v.type}`.toLowerCase()).length,
        outputFile: null,
    ...
    try {
        ...
        logger.info(`Total Number Of Pages: ${four_pdf_in_page.length} so estimated labels ${four_pdf_in_page.length * 4}`);
        ...
        for (let fileI in pdfIndexedBuffers) {
            ...
            fs.writeFileSync(templatePath, fs.readFileSync("./src/index_temp.html").toString().replaceAll("HELLO", buffer.index));
...
async function scissor(four_pdf_in_page, type) {
    let pdfBuffers = [];
    if (type === "Sevenoneone") {
        let four = [];
    ...
    for (let fileI in four_pdf_in_page) {
        ...
        logger.info(`Cutting label of 4 pages Current ${fileI} of ${four_pdf_in_page.length} AIrway Bills Number ${pdfBuffers.length}`);
        let pdfTempFolder = path.resolve(tempDir, "pdfs");
...
async function scisoorInner(file, pdfBuffers, x, y, w, h) {
    ...
    let cmd = `${exec}  -o ${dest} -sDEVICE=pdfwrite  -c "[/CropBox [${x}  ${y} ${x + w} ${y + h}] /PAGES pdfmark"  -f ${src}`;
    // logger.info(cmd)
    execSync(cmd);
